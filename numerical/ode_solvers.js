export class ODESolvers {
    ...
    let result = [[t0, ...Y0]];
    let t = t0;
    let vars = [...Y0];  // Copy initial values to avoid mutation

    if (verbose) console.log(`Euler Method Start: t0 = ${t0}, h = ${h}, steps = ${steps}`);

    for (let i = 0; i < steps; i++) {
        let derivatives = f(t, vars);  // Pass vars as an array
        if (verbose && i % 100 === 0) {
...
static rk4(f, t0, Y0, h, steps, verbose = false) {
    ...
    for (let i = 0; i < steps; i++) {
        let k1 = f(t, vars);
        let k2 = f(t + h / 2, vars.map((v, idx) => v + (h / 2) * k1[idx]));
        let k3 = f(t + h / 2, vars.map((v, idx) => v + (h / 2) * k2[idx]));
        let k4 = f(t + h, vars.map((v, idx) => v + h * k3[idx]));
